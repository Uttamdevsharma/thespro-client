...
const CommitteePendingProposalsPage = () => {
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {Array.from({ length: 6 }).map((_, i) => (
                    <div key={i} className="bg-white dark:bg-gray-900 p-8 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800">
  ...
  if (isError) {
    return (
        <div className="p-8 bg-gray-50 dark:bg-gray-950 min-h-screen flex items-center justify-center">
            <div className="bg-white dark:bg-gray-900 p-12 rounded-2xl shadow-xl max-w-lg w-full text-center border border-red-500/20">
                <AlertCircle size={64} className="mx-auto text-red-500 mb-6" />
                <h1 className="text-2xl font-extrabold text-gray-900 dark:text-gray-50 mb-3">Fetch Error</h1>
                <p className="text-gray-500 dark:text-gray-400 mb-8 italic font-medium">
      ...
      <div className="p-8 bg-gray-50 dark:bg-gray-950 min-h-screen">
        <div className="max-w-4xl mx-auto">
            <button 
                ...
                className="mb-8 flex items-center gap-2 px-4 py-2 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-200 font-bold rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:bg-gray-50 dark:bg-gray-950 transition-all group"
            >
                <ArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" /> Back to Research Cells
            </button>

            <div className="mb-8 border-l-4 border-green-500 pl-4 bg-white dark:bg-gray-900 p-6 rounded-r-2xl shadow-sm">
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Research Cell</p>
                <h1 className="text-3xl font-extrabold text-gray-900 dark:text-gray-50">{selectedCell.researchCell.title}</h1>
            </div>

            <div className="space-y-6">
            {selectedCell.proposals.map((proposal: any) => (
                <div key={proposal._id} className="bg-white dark:bg-gray-900 p-8 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800 transition-all hover:shadow-2xl">
                    <div className="flex justify-between items-start mb-6">
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-50 tracking-tight">{proposal.title}</h2>
                        <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">Pending</span>
                    </div>

                    <div className="bg-gray-50 dark:bg-gray-950 p-5 rounded-xl border border-gray-100 dark:border-gray-800 mb-6 font-medium text-gray-700 dark:text-gray-200 italic border-l-4 border-gray-300">
                        "{proposal.abstract}"
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div>
                            ...
                            {proposal.members.map((member: any) => (
                                <div key={member._id} className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-3 rounded-xl shadow-sm flex justify-between items-center text-sm">
                                    <span className="font-bold text-gray-800 dark:text-gray-100">{member.name}</span>
                                    <div className="flex items-center gap-2">
                                        <span className="text-gray-500 dark:text-gray-400 font-medium">{member.studentId}</span>
                        ...
                        <div>
                            ...
                            <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-4 rounded-xl shadow-sm space-y-3">
                                <div className="flex justify-between items-center text-sm">
                                    <span className="text-gray-500 dark:text-gray-400 font-medium">Lead Student:</span>
                                    <span className="font-bold text-gray-800 dark:text-gray-100">{proposal.createdBy.name}</span>
                                </div>
                                <div className="flex justify-between items-center text-sm">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gray-100 dark:border-gray-800">
                        ...
                        <button
                            className="flex-1 bg-white dark:bg-gray-900 hover:bg-red-50 text-red-600 border-2 border-red-500 font-bold py-3.5 px-6 rounded-xl transition-all flex items-center justify-center disabled:bg-gray-100"
        ...
        {proposalsByCell && proposalsByCell.length > 0 ? (
            ...
            {proposalsByCell.map((cell: any) => (
                <div
                    ...
                    className="bg-white dark:bg-gray-900 p-8 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800 cursor-pointer overflow-hidden relative group transition-all hover:shadow-2xl hover:-translate-y-1"
                ...
                >
                    ...
                    <h2 className="text-xl font-extrabold text-gray-900 dark:text-gray-50 mb-4 group-hover:text-green-700 transition-colors uppercase tracking-tight">{cell.researchCell.title}</h2>
