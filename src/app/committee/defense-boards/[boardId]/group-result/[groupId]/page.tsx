      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-5 mb-10">
            ...
            <div>
                ...
                <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mt-1 uppercase tracking-widest">Aggregate score breakdown for assigned unit</p>
            </div>
        </div>

        {summarizedResults.length > 0 ? (
          <div className="bg-white dark:bg-gray-900 shadow-2xl rounded-3xl overflow-hidden border border-gray-100 dark:border-gray-800 relative">
                    ...
                    <tr key={student.studentId} className="hover:bg-green-50/20 transition-all group">
                        <td className="px-8 py-6 whitespace-nowrap">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="font-extrabold text-gray-800 dark:text-gray-100 group-hover:text-green-700 transition-colors uppercase tracking-tight">{student.name}</p>
                                    <p className="text-[10px] font-black text-gray-400 font-mono tracking-tighter">{student.studentId}</p>
                                </div>
                            </div>
                        </td>
                        <td className="px-8 py-6 text-center">
                            <div className="inline-flex items-center space-x-2 bg-gray-50 dark:bg-gray-950 px-4 py-2 rounded-xl border border-gray-100 dark:border-gray-800 group-hover:bg-white dark:bg-gray-900 transition-all">
                                <UserCheck size={14} className="text-blue-400" />
                                <span className="text-sm font-black text-gray-700 dark:text-gray-200">{student.supervisorMark}</span>
                            </div>
                        </td>
                        <td className="px-8 py-6 text-center">
                            <div className="inline-flex items-center space-x-2 bg-gray-50 dark:bg-gray-950 px-4 py-2 rounded-xl border border-gray-100 dark:border-gray-800 group-hover:bg-white dark:bg-gray-900 transition-all">
                                <ShieldCheck size={14} className="text-purple-400" />
                                <span className="text-sm font-black text-gray-700 dark:text-gray-200">{student.avgCommitteeMark}</span>
                            </div>
                        </td>
