      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                <label htmlFor="supervisionFilter" className="block text-sm font-bold text-gray-700 mb-2 flex items-center">
                ...
                <select
                    ...
                    className="block w-full bg-white border border-gray-300 rounded-lg py-3 px-4 text-sm font-medium text-gray-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-green-400 focus:border-green-500 transition-all font-semibold"
                >
            ...
            {/* Defense Type Filter */}
            <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                <label htmlFor="defenseTypeFilter" className="block text-sm font-bold text-gray-700 mb-2 flex items-center">
        ...
        {error ? (
            ...
            {defenseResults.length === 0 ? (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tr>
                        <th className="py-4 px-6 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">SL</th>
                        <th className="py-4 px-6 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Students</th>
                        <th className="py-4 px-6 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Project Title</th>
                        <th className="py-4 px-6 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Type</th>
                        <th className="py-4 px-6 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Board Members</th>
                        <th className="py-4 px-6 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Comments</th>
                    </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                    {defenseResults.map((result, index) => (
                        <tr key={result._id} className="hover:bg-gray-50 transition-colors">
                        <td className="py-4 px-6 text-sm font-bold text-gray-900">{index + 1}</td>
                        <td className="py-4 px-6">
                            <div className="flex flex-col gap-1">
                                {result.students.map((s: any) => (
                                    <div key={s.studentId} className="flex items-center text-xs">
                                        <span className="bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded font-mono font-bold mr-2">{s.studentId}</span>
                                        <span className="text-gray-900 font-medium">{s.name}</span>
                                    </div>
                                ))}
                            </div>
                        </td>
                        <td className="py-4 px-6 text-sm font-semibold text-gray-900">{result.title}</td>
                        <td className="py-4 px-6">
                        ...
                        <td className="py-4 px-6 text-sm text-gray-600 font-medium">
                            <div className="flex flex-wrap gap-1">
                                {result.boardMembers.map((m: string, i: number) => (
                                    <span key={i} className="bg-gray-50 px-2 py-1 rounded border border-gray-100">{m}</span>
                                ))}
                            </div>
                        </td>
                        <td className="py-4 px-6 text-sm">
                            <div className="space-y-3 max-w-xs">
                                ...
                                <div key={i} className="bg-green-50 p-2 rounded-lg border border-green-100">
                                    ...
                                    <p className="text-gray-800 text-sm italic">"{c.text}"</p>
                                </div>
                                ))}
                            </div>
                        </td>
