  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Filter Section */}
        <div className="mb-8 max-w-sm bg-white p-5 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="space-y-10">
            {defenseBoards && defenseBoards.length > 0 ? (
              (defenseBoards as any[]).map(board => (
                <div key={board._id} className="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100 transition-all hover:shadow-2xl">
                  {/* Board Header Info */}
                  <div className="bg-gradient-to-r from-gray-50 to-white p-6 border-b border-gray-200">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-sm">
                        <div className="flex items-center">
                            ...
                            <span className="text-gray-500 font-medium">Board Number</span>
                        </div>
                        <div className="flex items-center">
                            <Calendar size={18} className="text-blue-500 mr-2" />
                            <span className="text-gray-700 font-semibold">{new Date(board.date).toLocaleDateString(undefined, { dateStyle: 'long' })}</span>
                        </div>
                        <div className="flex items-center">
                            <MapPin size={18} className="text-red-500 mr-2" />
                            <span className="text-gray-700 font-semibold">{board.room?.name || 'N/A'}</span>
                        </div>
                        <div className="flex items-center">
                            <Clock size={18} className="text-purple-500 mr-2" />
                            <span className="text-gray-700 font-semibold">{board.schedule ? `${board.schedule.startTime} - ${board.schedule.endTime}` : 'N/A'}</span>
                        </div>
                    </div>
                  </div>
